...
let newsPullBackgroundTimer;
let staleStoryDeleteBackgroundTimer;
...
function refreshStories(doc, callback) {
  ...
  for (let filterIdx = 0; filterIdx < doc.newsFilters.length; filterIdx++) {
    ...
    if (
      "keyWords" in doc.newsFilters[filterIdx] &&
      ...
      for (let i = 0; i < doc.newsFilters[filterIdx].keyWords.length; i++) {
        for (let j = 0; j < globalNewsDoc.newsStories.length; j++) {
          if (globalNewsDoc.newsStories[j].keep == false) {
            let s1 = globalNewsDoc.newsStories[j].title.toLowerCase();
            let s2 = globalNewsDoc.newsStories[j].contentSnippet.toLowerCase();
            let keyword = doc.newsFilters[filterIdx].keyWords[i].toLowerCase();
            if (s1.indexOf(keyword) >= 0 || s2.indexOf(keyword) >= 0) {
              globalNewsDoc.newsStories[j].keep = true;
              storiesMatched++;
            }
          }
          if (storiesMatched == process.env.MAX_FILTER_STORIES) break;
        }
        if (storiesMatched == process.env.MAX_FILTER_STORIES) break;
      }
    ...
    function(n, next) {
      setTimeout(function() {
        ...
        try {
            ...
            .get(
              {
                ...
                path:
                  "/svc/topstories/v2/" +
                  NEWYORKTIMES_CATEGORIES[n] +
                  ".json?api-key=" +
                  process.env.NEWYORKTIMES_API_KEY
          ...
          if (count == 3) {
            console.log(
              "app_FORK.js: shuting down timer...too many errors: err:" + err
            );
            clearInterval(newsPullBackgroundTimer);
            clearInterval(staleStoryDeleteBackgroundTimer);
            process.disconnect();
        ...
        db.collection.findOne({ _id: process.env.GLOBAL_STORIES_ID }, function(
          ...
          if (err) {
            console.log({
              msg: "FORK_ERROR",
              Error:
                "Error with the global news doc read request: " +
            ...
            for (let i = 0; i < results.length; i++) {
              ...
              try {
                let news = JSON.parse(results[i]);
              } catch (e) {
                console.error(e);
                return;
              }
              for (let j = 0; j < news.results.length; j++) {
                let xferNewsStory = {
...
function refreshAllUserStories() {
  db.collection.findOneAndUpdate(
    ...
    {
      $set: {
        newsStories: globalNewsDoc.newsStories,
    ...
    function(err, result) {
      if (err) {
        ...
        let cursor = db.collection.find({ type: "USER_TYPE" });
        ...
        async.doWhilst(
          ...
          function(err) {
            console.log(
              "Timer: News stories refreshed and user newsFilters matched. err:",
              err
            );
          }
        );
  ...
  db.collection.find({ type: "SHAREDSTORY_TYPE" }).toArray(function(err, docs) {
    ...
    async.eachSeries(
      ...
      function(story, innercallback) {
        ...
        let d1 = story.comments[0].dateTime;
        let d2 = Date.now();
        let diff = Math.floor((d2 - d1) / 3600000);
        if (diff > 72) {
